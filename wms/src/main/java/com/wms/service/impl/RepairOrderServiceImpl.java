...
public class RepairOrderServiceImpl extends ServiceImpl<RepairOrderMapper, RepairOrder>
    ...
    public Result getOrdersByMechanicId(Integer mechanicId, String status) {
        try {
            LambdaQueryWrapper<RepairOrder> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(RepairOrder::getMechanicId, mechanicId);
            ...
            return Result.fail("查询技师任务失败: " + e.getMessage());
        }
    }

    @Override
    public Result getMechanicPendingTasks(Integer mechanicId) {
        try {
            LambdaQueryWrapper<RepairOrder> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(RepairOrder::getMechanicId, mechanicId)
                    .in(RepairOrder::getStatus,
                            Arrays.asList(STATUS_REPAIRING, STATUS_INSPECTING, STATUS_QUOTE_PENDING))
                    .orderByAsc(RepairOrder::getCreateTime); // 按创建时间升序，先来的先处理
            ...
            return Result.fail("查询待处理任务失败: " + e.getMessage());
        }
    }
}