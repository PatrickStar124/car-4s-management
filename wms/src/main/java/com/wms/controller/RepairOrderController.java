...
public class RepairOrderController {
    ...
    /**
     * 技师查询自己待处理的任务
     * 用途：技师端快速查看需要处理的工单（维修中、检查中、待报价）
     * @param mechanicId 技师ID
     * @return 待处理任务列表
     */
    @GetMapping("/mechanic/{mechanicId}/pending")
    public Result getMechanicPendingTasks(@PathVariable Integer mechanicId) {
        return repairOrderService.getMechanicPendingTasks(mechanicId);
    }

    // 或者添加一个根据当前登录用户查询的接口（更安全）
    /**
     * 获取当前技师的任务（从token/session获取技师ID）
     * 用途：技师登录后查看自己的任务，无需传递ID参数
     */
    @GetMapping("/my-tasks")
    public Result getMyTasks(@RequestParam(required = false) String status) {
        // 这里需要从session或token中获取当前用户ID
        // 假设你有一个工具类可以获取当前用户
        // Integer currentUserId = AuthUtils.getCurrentUserId();

        // 暂时注释掉，如果你有用户认证系统再实现
        // return repairOrderService.getOrdersByMechanicId(currentUserId, status);

        return Result.fail("请先实现用户认证系统");
    }
}